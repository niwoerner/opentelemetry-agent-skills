...
A global `retry` block applies to detector failures. Azure Container Apps (`azurecontainerapps`)
and, since v0.160.0, Azure App Service (`azureappservice`) are available as detectors. Retries are
enabled by default, with the overall default session bounded by the 5s HTTP timeout.
...
- [Configuration](configuration.md) — top-level keys (`detectors`, `override`, `refresh_interval`, `retry`, HTTP client settings), the per-detector `resource_attributes` enable/disable mechanism, and the full detector catalog with the attributes each populates.
...
- [Known quirks](quirks.md) — a failed detector stops the Collector from starting, the `override` footgun, "describes the Collector's host, not the workload", `refresh_interval` cardinality cost, v0.157.0 internal telemetry, the `resourcedetection` deprecated alias, and Docker/cloud environment caveats.
