...
> The exporter `http.endpoint` host must match the Prometheus container's network alias (`prw-prom` here). A wrong host doesn't fail fast — the push hangs until the request timeout and the collector logs `Permanent error: context deadline exceeded`, `Dropping data`.
...
- **`http.tls.insecure: true`** → plaintext push works against the receiver (TLS is on by default otherwise).
