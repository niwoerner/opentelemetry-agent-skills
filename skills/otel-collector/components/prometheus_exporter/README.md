...
Avoid when:
- You need to **push** metrics out — use the [`prometheus_remote_write`](../prometheus_remote_write/README.md) exporter.
...
- [`prometheus_remote_write`](../prometheus_remote_write/README.md) exporter — the **push** sibling; pick it when you need to send metrics out via Remote Write rather than be scraped.
...
- [Verification](verification.md) — drive an OTLP pipeline with `telemetrygen` and `curl` the exporter's own `/metrics`, proving namespace, suffixing, `const_labels`, timestamps, and latest-value accumulation on contrib v0.154.0. It does not exercise `resource_constant_labels`, added in v0.160.0.
