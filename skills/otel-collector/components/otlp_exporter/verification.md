...
go run github.com/open-telemetry/opentelemetry-collector-contrib/cmd/telemetrygen@v0.159.0 \
  traces --otlp-endpoint localhost:4317 --otlp-insecure --traces 5 --workers 1
```
...
`--traces N` counts **traces, not spans** — `telemetrygen` emits **2 spans per trace** (a parent and a child), so `--traces 5 --workers 1` produces **10 span records**, all of which should reach B:
