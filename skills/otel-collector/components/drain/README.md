...
Applies the [Drain](https://jiemingzhu.github.io/pub/pjhe_icws2017.pdf) log-clustering algorithm to each log record, derives a **template string** (e.g. `user <*> logged in from <*>`), and writes it to a configurable attribute (`log.record.template` by default). The template becomes a stable key for grouping, filtering, routing, or sampling whole classes of logs. Since v0.158.0, ordered regex masking rules can turn high-cardinality values into named template tokens and attributes; optional wildcard emission writes Drain's remaining `<*>` values to a string-slice attribute.
...
- You need named regex-matched values or the remaining positional wildcard values as structured attributes.
...
- [Configuration](configuration.md) — config keys, defaults, named masking, wildcard emission, validation rules, and structured-body behavior. Open when wiring up `drain` or tuning what gets templated.
...
- [Advanced use-cases](advanced.md) — persistence, granularity tuning, warmup, named masking, wildcard emission, delimiters, and combining with downstream components. Open when going beyond a single stock instance.
